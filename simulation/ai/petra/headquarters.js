...
PETRA.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	...
	if (!this.canBuild(gameState, "structures/{civ}/farmstead"))
		return;
    let numFarms = gameState.getOwnStructures().filter(API3.Filters.byClass("Field")).length;
    let numFarmstead = gameState.getOwnStructures().filter(API3.Filters.byClass("Farmstead")).length;
    if ((numFarms / numFarmstead) < 5)
        return;
