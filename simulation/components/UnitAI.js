		...
		"GATHER": {
			...
			"GATHERING": {
				...
				"Timer": function(msg) {
					...
					if (status.exhausted)
					{
						if (this.order.data.type.specific == "grain")
						{
							let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
							if (!cmpPosition || !cmpPosition.IsInWorld())
								return;

							let nearbyFoundation = this.FindNearbyField(this.TargetPosOrEntPos(msg.data.newentity));
							if (nearbyFoundation)
							{
								this.AddOrder("Repair", { "target": nearbyFoundation, "autocontinue": true, "force": true }, true);
								return;
							}
		...
		"REPAIR": {
			...
			"REPAIRING": {
				"enter": function() {
					...
					let isField = Engine.QueryInterface(this.repairTarget, IID_Identity).HasClass("Field");
					if(isField)
						this.SelectAnimation("gather_grain");
					else
...
UnitAI.prototype.FindNearbyField = function(position)
{
	if (!position)
		return undefined;
	...
	// we're working on has been deleted.)
	return nearby.find(ent => !Engine.QueryInterface(ent, IID_Foundation).IsFinished() 
		&& Engine.QueryInterface(ent, IID_Identity).HasClass("Field"));
};
